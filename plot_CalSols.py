...
def phase_or_amp(sel_offset,data):
	...
	if sel_offset == 2:
		print "plotting Jones phases for %d frequency channels and %d antennas" % (N_ch, N_ant)
		
		# unwrap 2pi phase jumps
		for ant in range(0, N_ant):
			last_PX_lsq_phase = PX_lsq[ant][1]
			last_PY_lsq_phase = PY_lsq[ant][1]
			last_QX_lsq_phase = QX_lsq[ant][1]
			last_QY_lsq_phase = QY_lsq[ant][1]
			last_PX_fit_phase = PX_fit[ant][1]
			last_PY_fit_phase = PY_fit[ant][1]
			last_QX_fit_phase = QX_fit[ant][1]
			last_QY_fit_phase = QY_fit[ant][1]
			PX_lsq[ant][1] *= 180/pi
			PY_lsq[ant][1] *= 180/pi
			QX_lsq[ant][1] *= 180/pi
			QY_lsq[ant][1] *= 180/pi
			...
			for offset in range(3,2*N_ch,2):
	...
	# --------------------------------------------------------------------------------- #
	# Additioal Infos file

	outtile = ('Add_Info_%s.txt' %obs)
	...
	if sel_offset == 1:
		...
		savefig('StDev_plots_%s.png'%obs, bbox_inches='tight', pad_inches=1) #,bbox_inches='tight'
		plt.close(fig)
		#
		# ================================================	
		# Check for spikes
	...
	for ii in argo:
		...
		if str(ii) in flg_t: # plot flagged boxes
			...
			if sel_offset == 2:
				XX_phase, = ax.plot(freq[freq_idx],(((PX_lsq[ii][chan_sel]-PX_lsq[argo[refa]][chan_sel]) + 180) % (2 * 180) - 180), 'o',
					ms=1, lw=0., markeredgecolor=colours[3],color=colours[3],label='XX')
				YY_phase, = ax.plot(freq[freq_idx],(((QY_lsq[ii][chan_sel]-QY_lsq[argo[refa]][chan_sel]) + 180) % (2 * 180) - 180), 'o', 
					ms=1, lw=0., markeredgecolor=colours[0],color=colours[0],label='YY')
				ax.set_xlim(min(freq[freq_idx]), max(freq[freq_idx]))
		...
		if sel_offset == 2: plt.ylim([-180,180])
	...
	if sel_offset == 2:
		plt.suptitle('Phases | Reference %s | Calibrator %s | JD %s' %(tiles[argo[refa]],calid,JD),fontsize=18)
		savefig('Phases_%s.png'%obs, bbox_inches='tight') #,bbox_inches='tight'
...
def make_the_plot(cal_loc):
	...
	if(options.ignore_flagged_tiles_text):
		flg_t = []
	else:
		try:
	...
	try:
		hdulist = pyfits.open(cal_loc+obs+'.metafits')
	except:
		hdulist = pyfits.open(cal_loc+obs+'_metafits_ppds.fits')
...
#		dg = np.ones(len(gains[0,:]))
	...
	# loop on DI-files (same number of files of BP files)
		ggo.append(DI_factors(di_file[bbb]))
	...
	try:
		...
		print 'Instr_config file not found - reading Tile flags from meafits file'
	...
	if sel_offset == 0 or sel_offset == 1: phase_or_amp(1,data)
